...
class GaussianModel:
    ...
    def apply_ct(self, color_ct: np.ndarray):
        ...
        # self._features_dc.data = sh_data[:, 0:1, :].stop_grad()
        # self._features_rest.data = sh_data[:, 1:, :].stop_grad()
        self._features_dc = sh_data[:, 0:1, :].stop_grad()
        self._features_rest = sh_data[:, 1:, :].stop_grad()
        # self._features_dc.data.copy_(sh_data[:,0:1,:])
        # self._features_rest.data.copy_(sh_data[:,1:,:])
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        # self.max_radii2D = jt.zeros((self.get_xyz.shape[0]), device="cuda")
    ...
    def prune_over_opacity_points(self, max_opacity, min_opacity, all_dir:bool=True):
        if all_dir:
            ...
            print((self.get_opacity.abs().max(dim=1)< max_opacity))
            print((self.get_opacity.abs().max(dim=1)< max_opacity).shape)
            print(self.get_opacity.abs().max(dim=1) > min_opacity)
        ...
        print((self.get_opacity.abs().max(dim=1)))
        ...
        self.denom = jt.zeros((self.get_xyz.shape[0], 1))
        self.max_radii2D = jt.zeros((self.get_xyz.shape[0]))
        print(selected_pts_mask.logical_not().shape)
