...
def searchsorted(a, v):
  ...
  Returns:
    ...
    last index of a.
  """
    i = jt.arange(a.shape[-1], device=a.device)
    v_ge_a = v[..., None, :] >= a[..., :, None]
    idx_lo = jt.max(jt.where(v_ge_a, i[..., :, None], i[..., :1, None]), -2).values
    idx_hi = jt.min(jt.where(~v_ge_a, i[..., :, None], i[..., -1:, None]), -2).values
...
def query(tq, t, y, outside_value=0):
    ...
    yq = jt.where(idx_lo == idx_hi, jt.full_like(idx_hi, outside_value),
                     jt.take_along_dim(y, idx_lo, dim=-1))
    return yq


def inner_outer(t0, t1, y1):
    """Construct inner and outer measures on (t1, y1) for t0."""
    cy1 = jt.cat([jt.zeros_like(y1[..., :1]),
                     jt.cumsum(y1, dim=-1)],
                    dim=-1)
    idx_lo, idx_hi = searchsorted(t1, t0)

    cy1_lo = jt.take_along_dim(cy1, idx_lo, dim=-1)
    cy1_hi = jt.take_along_dim(cy1, idx_hi, dim=-1)

    y0_outer = cy1_hi[..., 1:] - cy1_lo[..., :-1]
    y0_inner = jt.where(idx_hi[..., :-1] <= idx_lo[..., 1:],
                           cy1_lo[..., 1:] - cy1_hi[..., :-1], jt.zeros_like(idx_lo[..., 1:]))
...
def max_dilate(t, w, dilation, domain=(-jt.inf, jt.inf)):
    """Dilate (via max-pooling) a non-negative step function."""
    t0 = t[..., :-1] - dilation
    t1 = t[..., 1:] + dilation
    t_dilate, _ = jt.sort(jt.cat([t, t0, t1], dim=-1), dim=-1)
    t_dilate = jt.clip(t_dilate, *domain)
    w_dilate = jt.max(
        jt.where(
            (t0[..., None, :] <= t_dilate[..., None])
            & (t1[..., None, :] > t_dilate[..., None]),
            w[..., None, :],
            jt.zeros_like(w[..., None, :]),
        ), dim=-1).values[..., :-1]
...
def max_dilate_weights(t,
                       w,
                       dilation,
                       domain=(-jt.inf, jt.inf),
                       renormalize=False):
    """Dilate (via max-pooling) a set of weights."""
    eps = jt.finfo(w.dtype).eps
    # eps = 1e-3
    ...
    if renormalize:
        w_dilate /= jt.sum(w_dilate, dim=-1, keepdim=True).clamp_min(eps)
    return t_dilate, w_dilate
...
def integrate_weights(w):
  ...
  """
    cw = jt.cumsum(w[..., :-1], dim=-1).clamp_max(1)
    shape = cw.shape[:-1] + (1,)
    # Ensure that the CDF starts with exactly 0 and ends with exactly 1.
    cw0 = jt.cat([jt.zeros(shape, device=cw.device), cw,
                     jt.ones(shape, device=cw.device)], dim=-1)
    return cw0
  ...
  """
    ...
    if not rand:
        if deterministic_center:
            pad = 1 / (2 * num_samples)
            u = jt.linspace(pad, 1. - pad - eps, num_samples, device=device)
        else:
            u = jt.linspace(0, 1. - eps, num_samples, device=device)
        u = jt.broadcast_to(u, t.shape[:-1] + (num_samples,))
    else:
        # `u` is in [0, 1) --- it can be zero, but it can never be 1.
        u_max = eps + (1 - eps) / num_samples
        max_jitter = (1 - u_max) / (num_samples - 1) - eps
        d = 1 if single_jitter else num_samples
        u = jt.linspace(0, 1 - u_max, num_samples, device=device) + \
            jt.rand(t.shape[:-1] + (d,), device=device) * max_jitter
...
def sample_intervals(rand,
  ...
  """
    ...
    t_samples = jt.cat([first, mid, last], dim=-1)
    return t_samples
...
def interval_distortion(t0_lo, t0_hi, t1_lo, t1_hi):
    ...
    return jt.where(are_disjoint, d_disjoint, d_overlap)
...
def weighted_percentile(t, w, ps):
    ...
    fn = lambda cw_i, t_i: math.sorted_interp(jt.tensor(ps, device=t.device) / 100, cw_i, t_i)
...
def resample(t, tp, vp, use_avg=False):
  ...
  """
    ...
    acc = jt.cumsum(vp, dim=-1)
    acc0 = jt.cat([jt.zeros(acc.shape[:-1] + (1,), device=acc.device), acc], dim=-1)
    acc0_resampled = math.sorted_interp(t, tp, acc0)  # TODO
    v = jt.diff(acc0_resampled, dim=-1)
    return v
...
def blur_stepfun(x, y, r):
    ...
    y2 = jt.cat([y1, -y1], dim=-1).take_along_dim(xr_idx[..., :-1], dim=-1)
    yr = jt.cumsum((xr[..., 1:] - xr[..., :-1]) *
                      jt.cumsum(y2, dim=-1), dim=-1).clamp_min(0)
